...
| Description | Saha searches the web for a Hindu sloka or mantra by name, lists the versions it finds (grouped by script), transliterates the verses into another Indic script, can generate an English meaning, and exports a PDF. Meanings are AI-generated and labelled as such. Verse text is attributed to the source page. Differentiators: multi-script transliteration, side-by-side layout, on-device saved items, PDF export. |
...
> Saha looks up publicly available Hindu scripture (slokas/mantras) by name. On first search (and on Show meaning if the user has not already agreed) it shows a disclosure sheet: the typed name is sent to a web search provider, candidate pages are fetched, page text may be sent to a third-party AI provider to extract verses, and Show meaning sends verse text to a third-party AI provider. The user must tap Continue before any of those sends; Not now cancels. Permission can be withdrawn on the Search screen. It displays verse text found on third-party pages with a source link. It does not copy a site’s translation or commentary. English meanings are generated by our backend and shown with the disclaimer “AI-generated meaning; verify with a scholar.” There is no user login. Demo: accept the sheet, then type “Gayatri Mantra” on the Search screen. Privacy policy: the `/privacy` path on the same host as the API.

This addresses guideline 5.2.2 (third-party content) via attribution + original generated meaning, and 5.1.2(i) via the disclosure sheet.

Demo account: none (stateless). Backend must be live during review.

## 6. TestFlight

After processing, add internal testers (App Store Connect users) and install on a real iPhone. Check the consent sheet (Continue / Not now / withdraw), search, extract, transliteration, meaning, PDF share, and saved items. Fix crash/font issues, then cut a new build if needed (`eas build` + `eas submit`).
